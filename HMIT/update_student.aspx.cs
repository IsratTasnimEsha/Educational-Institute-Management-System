    ...
    public partial class update_student : System.Web.UI.Page
    {
            ...
            if (Session["email"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    if (Request.QueryString["studentID"] != null)
                    {
                        ...
                        con1.Open();
                        SqlCommand cmd1 = new SqlCommand("SELECT * FROM students WHERE student_id='" + studentID + "'", con1); // Replace SomeCondition with your actual condition
                        SqlDataReader reader1 = cmd1.ExecuteReader();

                        if (reader1.Read())
                        {
                            name.Text = reader1["student_name"].ToString();

                            string t_id = reader1["student_id"].ToString();
                            code.Text = "" + t_id[0] + t_id[1];
                            btch.Text = "" + t_id[2] + t_id[3];
                            id.Text = "" + t_id[4] + t_id[5] + t_id[6];
                            ...
                            fname.Text = reader1["father_name"].ToString();
                            mname.Text = reader1["mother_name"].ToString();
                            batch.Text = reader1["batch"].ToString();
                            ad.Text = reader1["s_address"].ToString();
        ...
        protected void dept_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (Session["email"] != null)
            {
                ...
                {
                    string studentID = HttpUtility.UrlDecode(Request.QueryString["studentID"]);
                    pass.Text = name.Text[0] + studentID;
                }
                else
                {
                    pass.Text = "";
        ...
        protected void submit_Click(object sender, EventArgs e)
        {
            if (Session["email"] != null)
            {
                ...
                phone.Text.Length != 11 || string.IsNullOrWhiteSpace(fname.Text) ||
                string.IsNullOrWhiteSpace(mname.Text) || string.IsNullOrWhiteSpace(ad.Text) ||
                ...
                else
                {
                    ...
                    SqlCommand cmd = new SqlCommand("update students set student_name='" + name.Text.Trim() + "', phone='" + phone.Text.Trim() + "', s_address='" + ad.Text.Trim() + "',batch='" + batch.Text.Trim() + "', father_name='" + fname.Text.Trim() + "', mother_name='" + mname.Text.Trim() + "', pass='" + encrypted + "', imageID = " + Convert.ToInt32(lblMessage.Text) + " where student_id='" + studentID + "'", con);
